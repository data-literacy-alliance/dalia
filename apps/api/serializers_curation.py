...
class ResourceContentWriteSerializer(serializers.ModelSerializer):
    ...
    uuid = serializers.UUIDField(read_only=True)
    uuid = serializers.UUIDField(read_only=True)
    # Maps the form's 'version' string key to version_label on the model.
    # The integer ResourceContent.version (edit counter) is made read-only via Meta.
    version = serializers.CharField(
        source="version_label", allow_blank=True, required=False, default=""
    )
