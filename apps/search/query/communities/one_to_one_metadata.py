...
def get_one_to_one_metadata_for_communities(
    community_uri_refs: Iterable[URIRef],
) -> Dict[URIRef, Community]:
    """
    ...
    return {
        result.community: process_result_for_one_to_one_metadata_for_community(result)
...
def process_social_media_in_result(result):
    ...
    if result.zenodo_community_id:
        social_media.append(
            SocialMedia(
                name="Zenodo",
                url="https://zenodo.org/communities/" + str(result.zenodo_community_id),
            )
        )
    if result.youtube_channel_id:
        social_media.append(
            SocialMedia(
                name="YouTube",
                url="https://www.youtube.com/channel/" + str(result.youtube_channel_id),
            )
        )
    if result.bluesky_handle:
        social_media.append(
            SocialMedia(
                name="Bluesky", url="https://bsky.app/profile/" + str(result.bluesky_handle)
            )
        )
    if result.mastodon_address:
        server = str(result.mastodon_address).split("@")[-1]
        handle = str(result.mastodon_address).split("@")[0]
        social_media.append(SocialMedia(name="Mastodon", url=f"https://{server}/@{handle}"))
    if result.linkedin_id:
        social_media.append(
            SocialMedia(
                name="LinkedIn", url="https://www.linkedin.com/company/" + str(result.linkedin_id)
            )
        )
...
def prepare_query_for_one_to_one_metadata_for_communities(
    community_uri_refs: Iterable[URIRef],
) -> str:
    ...
    return (
        ...
        .WHERE(
            VALUES([var_community], resource_uri_ref_blocks),
            ...
            OPTIONAL(
                (var_community, wdt.LinkedIn_company_or_organization_ID, _VARIABLES["linkedin_id"])
            ),
