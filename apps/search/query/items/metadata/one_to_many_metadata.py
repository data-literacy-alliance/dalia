...
_VARIABLES = {"lr": Variable("lr"), "item": Variable("item")}


def prepare_query_for_one_to_many_metadata_for_resources(
    resource_uri_refs: List[URIRef], relation: URIRef
) -> str:
...
def _metadata_from_results(
    ...
    # learning resource
    return {
        lr: _collect_sorted_labels(items, item_mapping) for lr, items in lr_metadata_items.items()
    }
