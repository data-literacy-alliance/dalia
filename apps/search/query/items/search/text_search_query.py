...
def prepare_where_for_text_search_for_learning_resources(
    ...
    if _is_browse_all_query(query):
        ...
        where = [
            ...
            OPTIONAL(
                (var_lr, SCHEMA.datePublished, var_created)
            ),
            FILTER_EXISTS(
        ...
        right_list = Collection(Graph(), BNode(), [Dalia_text.learningResourceTexts, Literal(lucene_query)])
        ...
        where = [
            ...
            OPTIONAL(
                ...
                OPTIONAL(
                    (var_s, SCHEMA.datePublished, var_created)
                ),
                BIND(
                    var_s,
                    var_lr
    ...
    for facet, active_facet_items in active_facets.items():
        if active_facet_items:
            ...
            where.append(
                VALUES(
                    [var_facet_value],
                    [(item,) for item in active_facet_items]
                )
            )
            where.append(
                (var_lr, facet.predicate, var_facet_value)
            )
    ...
    if date_published_after or date_published_before:
        ...
        where.append(
            (var_lr, SCHEMA.datePublished, var_date)
        )

        if date_published_after:
            year = date_published_after[:4]
            year_month = date_published_after[:7]
            where.append(FILTER(_MultiTypeDateExpr(
                ">=", date_published_after, year_month, year, var_date
            )))
        ...
        if date_published_before:
            ...
            where.append(FILTER(_MultiTypeDateExpr(
                "<=", date_published_before, year_month, year, var_date
            )))
