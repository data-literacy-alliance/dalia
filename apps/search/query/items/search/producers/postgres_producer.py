...
def _apply_text_query(qs, query: Optional[str]):  # noqa: ANN001
    ...
    Searches the STORED ``search_vector`` (title + description) AND keyword
    tags (exact case-insensitive match).  Results from both branches are
    merged by PK so no .distinct() JOIN is needed.  FTS matches carry a real
    SearchRank; keyword-only matches get rank 0.0.
    """
    q = (query or "").strip()
    if q in _BROWSE_ALL:
        return qs, False
    ...
    # FTS match PKs (title + description via stored search_vector)
    fts_pks = frozenset(qs.filter(search_vector=search_query).values_list("pk", flat=True))
    # Keyword exact match PKs (case-insensitive, covers PG-only resources)
    kw_pks = frozenset(qs.filter(keywords__name__iexact=q).values_list("pk", flat=True))
    # Person/org author name search (covers click-through from author badges)
    parts = q.split()
    if len(parts) >= 2:
        p_filter = (
            Q(people__first_name__icontains=parts[0]) & Q(people__last_name__icontains=parts[-1])
        ) | (Q(people__first_name__icontains=parts[-1]) & Q(people__last_name__icontains=parts[0]))
    else:
        p_filter = Q(people__first_name__icontains=q) | Q(people__last_name__icontains=q)
    author_pks = frozenset(
        qs.filter(p_filter | Q(organizations__name__icontains=q)).values_list("pk", flat=True)
    )
    all_pks = fts_pks | kw_pks | author_pks

    if not all_pks:
        return qs.none(), True

    if fts_pks:
        result_qs = qs.filter(pk__in=all_pks).annotate(
            _rank=Case(
                When(pk__in=list(fts_pks), then=SearchRank(F("search_vector"), search_query)),
                default=Value(0.0),
                output_field=FloatField(),
            )
        )
    else:
        result_qs = qs.filter(pk__in=all_pks).annotate(_rank=Value(0.0, output_field=FloatField()))

    return result_qs, True
...
def _map_content(content: ResourceContent) -> Resource:
    ...
    return Resource(
        ...
        learning_time=None,
        versions=None,
        version=content.version_label or None,
        source="postgres",
