...
def _make_discipline_resolver() -> Callable[[object], Optional[str]]:
    """Return the discipline's own URI unconditionally.

    Unlike _make_uri_resolver which only returns URIs present in the canonical
    DISCIPLINE_FACET.items dict (n0-n9), this resolver returns any valid
    discipline URI so that sub-disciplines (e.g. n148, n40, n032) are included
    in build_reverse_facet_map and can be used as filter values in PG search.
    """
...
FACET_VOCAB_MAP: Dict[FacetObject, FacetVocabMapping] = {
    ...
    DISCIPLINE_FACET: FacetVocabMapping(
        model=Discipline,
        resolver=_make_discipline_resolver(),
