...
# ---------------------------------------------------------------------------
# Operator-aware URI resolution helpers
# ---------------------------------------------------------------------------


def _needs_operator_split(selected_facets, cross_ops) -> bool:
    """Return True if any operator deviates from the defaults (within=OR, cross=AND)."""
    if any(op.upper() == "OR" for op in cross_ops):
        return True
    for sf in selected_facets:
        if (getattr(sf, "operator", None) or "OR").upper() == "AND":
            return True
    return False


def _fuseki_uris_with_operators(
    text_query,
    selected_facets,
    cross_ops,
    sort_by,
    ...
    """Compute matching URIs applying per-facet AND/OR within-group operators
    and cross-facet AND/OR operators.

    Called only when the operator config deviates from the default
    (within=OR, cross=AND).  Within-facet AND is implemented by intersecting
    per-value URI sets; cross-facet OR by unioning per-group URI sets.

    The result list is not guaranteed to preserve SPARQL sort order — the
    caller is responsible for pagination over the returned list.
    """
    from search.api_models.api_models import SelectedFacet as _SF

    def _uris_for_group(sf) -> set:
        within_op = (getattr(sf, "operator", None) or "OR").upper()
        if within_op == "AND" and len(sf.selected) > 1:
            # Intersect per-value URI sets (resource must match all selected values)
            combined = None
            for val in sf.selected:
                sf_single = _SF(key=sf.key, selected=[val])
                active = extract_active_facets_from_selected_facets(
                    [sf_single], _ITEM_SEARCH_FACETS_MAPPED_BY_KEY
                )
                uris = set(
                    _get_matching_uris(
                        text_query,
                ...
                combined = uris if combined is None else combined & uris
            return combined if combined is not None else set()
        else:
            # OR within group (default): one SPARQL call with all selected values
            active = extract_active_facets_from_selected_facets(
                [sf], _ITEM_SEARCH_FACETS_MAPPED_BY_KEY
            )
            return set(
                _get_matching_uris(
                    text_query,
    ...
    active_sf = [sf for sf in selected_facets if sf.selected]
    if not active_sf:
        active = extract_active_facets_from_selected_facets(
            selected_facets, _ITEM_SEARCH_FACETS_MAPPED_BY_KEY
        )
        return _get_matching_uris(
            text_query,
            active,
            sort_by,
            sort_order,
    ...
    group_uri_sets = [_uris_for_group(sf) for sf in active_sf]

    combined = group_uri_sets[0]
    for i, uri_set in enumerate(group_uri_sets[1:]):
        op = cross_ops[i] if i < len(cross_ops) else "AND"
        if op.upper() == "OR":
            combined = combined | uri_set
        else:
            combined = combined & uri_set
...
def fuseki_search(request: ItemSearchRequest) -> ItemSearchResult:
    ...
    # Step 1: Get matching URIs — use operator-aware path when needed
    if _needs_operator_split(selected_facets, cross_ops):
        all_uris = _fuseki_uris_with_operators(
            text_query,
            selected_facets,
            cross_ops,
            sort_by,
        ...
        # Default fast path: all cross-AND, all within-OR — single SPARQL query
...
def fuseki_candidates(request: ItemSearchRequest):
    ...
    if _needs_operator_split(selected_facets, cross_ops):
        # Operator-aware path: returns URI list (ordering deferred to merge coordinator)
        uri_list = _fuseki_uris_with_operators(
            text_query,
            selected_facets,
            cross_ops,
            sort_by,
        ...
        for lr in uri_list:
            ...
            candidates.append(
                Candidate(
                    uuid=uuid_str,
                    sort_key="0000-00-00",  # merge coordinator re-sorts by date
                    source="fuseki",
                    ref=lr,
                )
            )
    else:
        # Default fast path: all cross-AND, all within-OR
...
def _fetch_and_count_facets_from_results(
    ...
    with ThreadPoolExecutor(max_workers=9) as executor:
        ...
        for future in as_completed(future_to_facet):
            ...
            try:
                pairs = future.result()  # now (lr, value) tuples
                if facet is DISCIPLINE_FACET:
                    from search.query.items.search.producers.postgres_producer import (
                        _get_disc_ancestors_map,
                    )
                    from collections import defaultdict as _dd
                    from rdflib import URIRef as _URIRef

                    _ancestors_map = _get_disc_ancestors_map()
                    _disc_to_lrs: dict = _dd(set)
                    for _lr_uri, _disc_uri in pairs:
                        _disc_uri_str = str(_disc_uri)
                        for _anc_uri_str in _ancestors_map.get(
                            _disc_uri_str, frozenset({_disc_uri_str})
                        ):
                            _disc_to_lrs[_anc_uri_str].add(str(_lr_uri))
                    for _uri_str, _lr_set in _disc_to_lrs.items():
                        counts[DISCIPLINE_FACET][_URIRef(_uri_str)] = len(_lr_set)
                else:
                    for _lr, value in pairs:  # lr discarded for non-discipline facets
...
def _fetch_facet_values(uris: List[URIRef], predicates: List[URIRef]) -> List[Node]:
    ...
    return [(row.lr, row.value) for row in results]
    ...
    for facet_obj in _ITEM_SEARCH_FACETS:
        ...
        if facet_obj is COMMUNITY_FACET:
            ...
            if facet_obj is DISCIPLINE_FACET:
                from search.query.items.search.producers.postgres_producer import (
                    _build_discipline_aggregate_map,
                )
                ...
                for item_key, item_label in facet_obj.items.items():
                    ...
                    facet_items.append(
                        FacetItem(
                            label=item_label,
                            value=item_key_str,
                            active=item_key_str in active_items_str,
                        ...
                        if fi.value == root_uri:
                            if fi.children is None:
                                ...
                                FacetItem(
                                    ...
                                    value=active_uri,
                                    active=True,
                                    count=counts_by_str.get(active_uri, 0),
                                )
                            )
                            break
    ...
    for facet_list in facet_lists:
        for facet in facet_list:
            ...
            for item in facet.facetItems or []:
                ...
                # Carry children from whichever source provides them (first wins)
                if item.children and aggregated[cat_name][val]["children"] is None:
                    aggregated[cat_name][val]["children"] = item.children
                if item.hasChildren and not aggregated[cat_name][val]["hasChildren"]:
                    aggregated[cat_name][val]["hasChildren"] = item.hasChildren
