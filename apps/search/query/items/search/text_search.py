...
def search_items(request: ItemSearchRequest) -> ItemSearchResult:
    ...
    active_facets = extract_active_facets_from_selected_facets(selected_facets, _ITEM_SEARCH_FACETS_MAPPED_BY_KEY)
    ...
    search_result.facets = compile_facets_for_text_search(text_query, active_facets, _ITEM_SEARCH_FACETS)
...
_SORT_ORDER_LOOKUP = {
    "asc": FunctionExpressions.ASC,
    "dsc": FunctionExpressions.DESC
}
...
def prepare_query_for_text_search_for_learning_resources(
    ...
    if sort_by == "relevance":
        builder.ORDER_BY(
            sort_order_fn(var_score)
        )
    elif sort_by == "created":
        builder.ORDER_BY(
            sort_order_fn(
                FunctionExpressions(year_from_date, var_created)
            ),
            sort_order_fn(
                FunctionExpressions(month_from_date, var_created)
            ),
            sort_order_fn(
                FunctionExpressions(day_from_date, var_created)
            )
        )
