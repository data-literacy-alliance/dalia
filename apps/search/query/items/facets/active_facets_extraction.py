...
def extract_active_facets_from_selected_facets(
        selected_facets: List[SelectedFacet],
        facets_to_match_mapped_by_key: Dict[URIRef, FacetObject]
    ...
    for selected_facet in selected_facets:
        if target_facet := facets_to_match_mapped_by_key.get(URIRef(selected_facet.key)):
            active_facet_items = _extract_active_facet_items_from_selected_facet(selected_facet, target_facet)
...
def _extract_active_facet_items_from_selected_facet(
    ...
    return [
        selected_facet_item_as_node for selected_facet_item in selected_facet.selected
        if (selected_facet_item_as_node := target_facet.selected_facet_initializer(selected_facet_item))
