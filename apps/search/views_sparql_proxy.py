...
class SPARQLProxyView(View):
    ...
    def _proxy(self, query: str):
        ...
        if _SERVICE_CLAUSE.search(query):
            return _cors_headers(
                JsonResponse({"error": "SERVICE clause is not allowed"}, status=403)
            )

        query = _enforce_limit(query)

        base_url = getattr(
            settings, "DALIA_TRIPLESTORE_BASE_URL", "http://daliaproject_prod-fuseki_1:3030/"
        )
        ...
        try:
            with urllib.request.urlopen(req, timeout=30) as resp:
                ...
                return _cors_headers(
                    HttpResponse(body, content_type=content_type, status=resp.status)
                )
        except urllib.error.HTTPError as e:
            return _cors_headers(
                JsonResponse({"error": "Query failed", "details": str(e.reason)}, status=e.code)
            )
        except urllib.error.URLError as e:
            return _cors_headers(
                JsonResponse(
                    {"error": "Triplestore unavailable", "details": str(e.reason)}, status=503
                )
            )
        except TimeoutError:
