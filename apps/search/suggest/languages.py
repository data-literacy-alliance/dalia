...
def _where_for_text_search(
    query: str, var_lang: Variable, var_score: Variable, var_label: Variable
):
    subject_list_for_text_search = Collection(Graph(), BNode(), [var_lang, var_score, var_label])
    object_list_for_text_search = Collection(
        Graph(), BNode(), [SKOS_last_call.prefLabel, Literal(query)]
    )
...
def prepare_query_for_language_search_and_label_retrieval(
    ...
    return (
        ...
        .WHERE(
            *_where_for_text_search(query, var_lang, var_score, var_label),
            ...
            BIND(
                FunctionExpressions.COALESCE(var_iso639P1Code, Literal(False)),
...
def _search_languages_and_retrieve_labels(
    query: str, limit: int, offset: int
) -> List[LabelValueItem]:
...
def _process_result_from_metadata_retrieval(result) -> LabelValueItem:
    return LabelValueItem(value=str(result.lang), label=str(result.label))
