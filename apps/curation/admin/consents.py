...
class ResourceConsentAdmin(BaseModelAdmin):
    list_display = ("person", "resource_content_title", "consent_type", "status", "granted_at", "uuid")
    ...
    search_fields = (
        ...
        "person__first_name", "person__last_name", "uuid",
    )
    readonly_fields = (
        "uuid", "created", "modified", "granted_at", "withdrawn_at",
    ...
    def grant_consent(self, request, queryset):
        ...
        self.message_user(request, f"Granted consent for {queryset.filter(status='pending').count()} items.")
    ...
    def withdraw_consent(self, request, queryset):
        ...
        self.message_user(request, f"Withdrew consent for {queryset.filter(status='granted').count()} items.")
...
class ResourcePublishingConsentAdmin(BaseModelAdmin):
    list_display = (
        "resource_content_title", "consenting_person", "cc0_agreed",
        "data_processing_agreed", "is_active", "created", "uuid",
    )
    list_filter = (
        "cc0_agreed", "data_processing_agreed", "public_display_agreed",
        "analytics_agreed", "is_active", "created",
    )
    search_fields = (
        "resource_content__id", "resource_content__title",
        "resource_content__resource__id", "resource_content__resource__title",
        "consenting_person__first_name", "consenting_person__last_name", "uuid",
    )
    readonly_fields = (
        "uuid", "created", "modified", "consent_given_at", "withdrawn_at",
        "ip_address", "user_agent",
    )
    ...
    fieldsets = (
        ...
        ("Consents", {
            "fields": ("cc0_agreed", "data_processing_agreed", "public_display_agreed", "analytics_agreed"),
        }),
        ("Legal Framework", {
        ...
        ("Audit Trail", {
            "fields": ("consent_given_at", "ip_address", "user_agent", "withdrawn_at", "withdrawal_reason"),
            "classes": ("collapse",),
        }),
    )
    ...
    def withdraw_all_consents(self, request, queryset):
        ...
        self.message_user(request, f"Withdrew all consents for {queryset.filter(is_active=True).count()} items.")
