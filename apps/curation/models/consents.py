...
class ResourceConsent(UUIDMixin, TimeStampedModel):
    ...
    resource_content = models.ForeignKey(
        "curation.ResourceContent", on_delete=models.CASCADE, related_name="consents"
    )
    person = models.ForeignKey(
        "curation.Person", on_delete=models.CASCADE, related_name="resource_consents"
    )
    consent_type = models.CharField(
        max_length=50, choices=CONSENT_TYPES, help_text="Type of consent being tracked"
    )
    ...
    expires_at = models.DateTimeField(
        null=True, blank=True, help_text="When this consent expires (if applicable)"
    )
    ...
    consent_version = models.CharField(max_length=20, help_text="Version of consent terms")

    # Technical details
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address when consent was given"
    )
    user_agent = models.TextField(blank=True, help_text="Browser/client information")

    # Withdrawal details
    withdrawal_reason = models.TextField(
        blank=True, help_text="Reason given for withdrawal (optional)"
    )
...
class ResourcePublishingConsent(UUIDMixin, TimeStampedModel):
    ...
    resource_content = models.ForeignKey(
        "curation.ResourceContent", on_delete=models.CASCADE, related_name="publishing_consents"
    )
    consenting_person = models.ForeignKey(
        "curation.Person", on_delete=models.PROTECT, related_name="publishing_consents_given"
    )
    ...
    data_processing_agreed = models.BooleanField(
        default=False, help_text="Consented to data processing"
    )
    public_display_agreed = models.BooleanField(
        default=False, help_text="Consented to public display of content"
    )
    analytics_agreed = models.BooleanField(
        default=False, help_text="Consented to analytics tracking"
    )

    # Terms of Service
    tos_version = models.CharField(max_length=20, help_text="Version of Terms of Service accepted")
    privacy_policy_version = models.CharField(
        max_length=20, help_text="Version of Privacy Policy accepted"
    )

    # GDPR compliance
    gdpr_lawful_basis = models.CharField(
        max_length=50, choices=GDPR_LAWFUL_BASIS, default="consent"
    )
    data_retention_period = models.DurationField(
        null=True, blank=True, help_text="How long data will be retained"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Whether this consent is currently active"
    )
    withdrawn_at = models.DateTimeField(null=True, blank=True)
