...
class Review(UUIDMixin, TimeStampedModel):
    ...
    resource_content = models.ForeignKey(
        "curation.ResourceContent", on_delete=models.CASCADE, related_name="reviews"
    )
    ...
    terms_version = models.CharField(
        max_length=20, help_text="Version of ToS accepted when creating this review"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["resource_content", "reviewer"], name="unique_review_per_reviewer"
            )
        ]
    ...
    def is_complete(self):
        ...
        required_questions = ReviewQuestion.objects.filter(
            community=self.community, required=True, is_active=True
        )
    ...
    def get_score(self):
        ...
        rating_answers = self.answers.filter(
            question__question_type__in=["rating", "scale"], rating_answer__isnull=False
        )
...
class ReviewQuestion(UUIDMixin, TimeStampedModel, Activatable):
    ...
    choices = models.JSONField(
        default=list, help_text="Available choices for choice-type questions (JSON array)"
    )
    ...
    help_text_field = models.TextField(
        blank=True, db_column="help_text", help_text="Additional guidance for reviewers"
    )
...
class ReviewAnswer(UUIDMixin, TimeStampedModel):
    ...
    question = models.ForeignKey(ReviewQuestion, on_delete=models.PROTECT, related_name="answers")
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["review", "question"], name="unique_answer_per_question"
            )
        ]
    ...
    def clean(self):
        ...
        if self.rating_answer is not None and self.question.question_type in ["rating", "scale"]:
            if (
                self.rating_answer < self.question.min_value
                or self.rating_answer > self.question.max_value
            ):
