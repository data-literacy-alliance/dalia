...
class PageAdmin(BaseModelAdmin):
    ...
    def get_urls(self):
        urls = super().get_urls()
        custom = [
            path('<int:pk>/convert/', self.admin_site.admin_view(self._convert_view), name='pages-page-convert'),
        ]
        return custom + urls
    ...
    def _convert_view(self, request, pk):
        ...
        return HttpResponseRedirect(request.META.get('HTTP_REFERER', reverse('admin:pages_page_changelist')))
    ...
    def _run_conversion(self, request, obj):
        ...
        try:
            result = subprocess.run(
                ['pandoc', obj.docx_file.path, '-f', 'docx', '-t', 'markdown', '--wrap=none', '-o', tmp_path],
            ...
            self.message_user(request, f'{obj}: pandoc not installed. Rebuild the web container.', messages.ERROR)
