...
class EntityMapping(models.Model):
    ...
    fuseki_uuid = models.UUIDField(
        db_index=True, unique=True, help_text="UUID4 of the entity in Fuseki graph database"
    )
    ...
    import_candidate = models.BooleanField(
        default=False, help_text="Entity exists in Fuseki but not in PostgreSQL"
    )
    import_approved = models.BooleanField(
        default=False, help_text="Import from Fuseki to PostgreSQL has been approved"
    )
    ...
    sync_notes = models.TextField(
        blank=True, help_text="Notes about synchronization status, errors, or conflicts"
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=~(
                    models.Q(postgresql_uuid__isnull=True) & models.Q(fuseki_uuid__isnull=True)
                ),
            ...
            models.CheckConstraint(
                condition=~(
                    models.Q(import_candidate=True) & models.Q(postgresql_uuid__isnull=False)
                ),
    ...
    def clean(self):
        ...
        if (
            ...
            and self.postgresql_uuid
            and self.fuseki_uuid
            and self.postgresql_uuid != self.fuseki_uuid
        ):
    ...
    def __str__(self):
        ...
        if self.import_candidate:
            ...
            return (
                f"{status_display} {self.entity_type}: PG:{self.postgresql_uuid} → Fuseki:pending"
            )
        else:
...
class SyncLog(models.Model):
    ...
    created = models.DateTimeField(default=timezone.now, editable=False)
    created_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name="sync_logs")
