...
class EntityMappingAdmin(BaseModelAdmin):
    ...
    def import_candidates_view(self, request):
        ...
        candidates_list = EntityMapping.objects.filter(
        ...
        return TemplateResponse(
            request,
            'admin/entity_mapping/import_candidates.html',
            context
        )
    ...
    def discover_candidates_view(self, request):
        ...
        if request.method == 'POST':
            ...
            messages.success(
                request,
                f"Discovery complete! Found {total_found} new import candidates."
            )
        ...
        return TemplateResponse(
            request,
            'admin/entity_mapping/discover_candidates.html',
            context
        )
    ...
    def sync_summary_view(self, request):
        ...
        for entity_type, _ in EntityMapping.ENTITY_TYPE_CHOICES:
            ...
            mappings = EntityMapping.objects.filter(entity_type=entity_type, postgresql_uuid__isnull=False)
            # Count import candidates (entities waiting for PostgreSQL creation)
            import_candidates = EntityMapping.objects.filter(entity_type=entity_type, import_candidate=True)
        ...
        return TemplateResponse(
            request,
            'admin/entity_mapping/sync_summary.html',
            context
        )
    ...
    def mark_as_synced(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully marked {updated} mappings as synced.",
            messages.SUCCESS
        )
    ...
    def mark_for_resync(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully marked {updated} mappings for resync.",
            messages.SUCCESS
        )
    ...
    def approve_selected_imports(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully approved {approved} import candidates.",
            messages.SUCCESS
        )
    ...
    def reject_selected_imports(self, request, queryset):
        ...
        self.message_user(
            request,
            f"Successfully rejected {rejected} import candidates.",
            messages.SUCCESS
        )
    ...
    def preview_entity_transformation(self, request, queryset):
        ...
        if queryset.count() > 10:
            self.message_user(
                request,
                "Please select 10 or fewer items for preview.",
                messages.WARNING
            )
            return
        ...
        return HttpResponseRedirect(reverse('admin:entity_mapping_entitymapping_changelist') + '?preview=1')
    ...
    create_postgresql_entities.short_description = "Create PostgreSQL entities from approved candidates"
    ...
    def approve_all_import_candidates(self, request, queryset):
        ...
        all_pending = EntityMapping.objects.filter(
            import_candidate=True,
        ...
        if total_count == 0:
            self.message_user(
                request,
                "No pending import candidates found to approve.",
                messages.WARNING
            )
            return HttpResponseRedirect(reverse('admin:entity_mapping_entitymapping_changelist'))
        ...
        return TemplateResponse(
            request,
            'admin/entity_mapping/confirm_bulk_action.html',
            context
        )
    ...
    def cleanup_import_candidates(self, request, queryset):
        ...
        to_delete = EntityMapping.objects.filter(
            import_candidate=True,
            import_approved=False
        )
        ...
        if total_count == 0:
            self.message_user(
                request,
                "No import candidates found to clean up.",
                messages.WARNING
            )
            return HttpResponseRedirect(reverse('admin:entity_mapping_entitymapping_changelist'))
    ...
    def preview_entity_view(self, request, mapping_id):
        ...
        return TemplateResponse(
            request,
            'admin/entity_mapping/entity_preview.html',
            context
        )
    ...
    def create_entity_view(self, request, mapping_id):
        ...
        return HttpResponseRedirect(reverse('admin:entity_mapping_preview_entity', args=[mapping_id]))
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['discover_candidates_url'] = reverse('admin:entity_mapping_discover_candidates')
