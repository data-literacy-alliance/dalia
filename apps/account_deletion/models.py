...
class AccountDeletionRequest(models.Model):
    ...
    initiated_at = models.DateTimeField(
        auto_now_add=True, help_text="When the deletion request was created"
    )
    ...
    confirmed_at = models.DateTimeField(
        null=True, blank=True, help_text="When user confirmed the deletion via email link"
    )

    confirmation_ip = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address from which confirmation was made"
    )

    confirmation_user_agent = models.TextField(
        blank=True, help_text="User agent string from confirmation request"
    )

    # Expiry dates
    token_expires_at = models.DateTimeField(
        db_index=True, help_text="Token expiry (7 days from initiated_at)"
    )
    ...
    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When deletion was actually completed"
    )
    ...
    cancelled_at = models.DateTimeField(
        null=True, blank=True, help_text="When the request was cancelled"
    )
    ...
    user_snapshot = models.JSONField(
        default=dict, help_text="Complete snapshot of user accounts for GDPR audit trail"
    )
...
class AccountDeletionItem(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, help_text="Type of content (model)"
    )
    ...
    model_name = models.CharField(
        max_length=100, help_text="Name of the model (e.g., ResourceContent)"
    )

    app_label = models.CharField(max_length=100, default="", help_text="Django app label")

    content_summary = models.TextField(help_text="Brief description of content for admin review")

    content_data = models.JSONField(
        default=dict, help_text="Full snapshot of content for audit trail"
    )
    ...
    reviewed_at = models.DateTimeField(
        null=True, blank=True, help_text="When this item was reviewed"
    )

    # Execution tracking
    processed_at = models.DateTimeField(
        null=True, blank=True, help_text="When this item was actually processed"
    )
