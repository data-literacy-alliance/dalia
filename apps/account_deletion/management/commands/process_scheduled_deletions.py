...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if count == 0:
            self.stdout.write(self.style.SUCCESS("No deletions scheduled for processing"))
            return
        ...
        for req in ready_requests:
            ...
            try:
                AccountDeletionService.process_deletion(deletion_request=req, admin_user=admin_user)
                processed_count += 1
                self.stdout.write(self.style.SUCCESS(f"  Successfully processed request #{req.id}"))
            except Exception as e:
                error_count += 1
                self.stdout.write(self.style.ERROR(f"  Error processing request #{req.id}: {e}"))
