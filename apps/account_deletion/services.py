...
class AccountDeletionService:
        ...
        for app_label, model_name in cls.PERSONAL_DATA_MODELS:
            try:
                with transaction.atomic():
                    ...
                    user_fields = ['user', 'owner', 'created_by', 'submitted_by', 'reviewer', 'user_ptr']
    ...
    def cancel_deletion(
        ...
        if not deletion_request.can_be_cancelled:
            raise ValueError(
                f"Cannot cancel deletion request in status: {deletion_request.status}"
            )
        ...
        logger.info(
            f"Deletion request #{deletion_request.id} cancelled by {cancelled_by.username}"
        )
    ...
    def process_deletion(
        ...
        if pending_items > 0:
            raise ValueError(
                f"Cannot process deletion: {pending_items} items still pending review"
            )
        ...
        logger.info(
            f"Completed deletion of user {username} (request #{deletion_request.id})"
        )
    ...
    def _send_confirmed_email(cls, deletion_request: AccountDeletionRequest) -> None:
        ...
        user = deletion_request.user
        cancellation_url = f"{settings.SITE_URL}/api/account/delete-cancel/{deletion_request.confirmation_token}/"
