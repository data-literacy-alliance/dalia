...
// ---------------------------------------------------------------------------
// Helper: API DELETE via browser session
// ---------------------------------------------------------------------------
async function apiDelete(page: Page, path: string): Promise<void> {
  ...
  await page.evaluate(
    ...
    { url: `https://search.dalia.education${path}`, csrf: csrfToken }
  );
}
...
test.describe('Items new form — full workflow', () => {
  ...
  test('01 — create new resource with all fields', async ({ page }) => {
    ...
    state.personUuid = data.people[0]?.uuid ?? null;
  });
  ...
  test.afterAll(async ({ browser }) => {
    ...
    try {
      ...
      // Delete the guest person created during test 01 by UUID so subsequent
      // runs can create the same name again (unique_guest_person_name constraint).
      if (state.personUuid) {
        await apiDelete(page, `/api/curation/persons/${state.personUuid}/`);
      }
    } finally {
      await context.close();
    }
